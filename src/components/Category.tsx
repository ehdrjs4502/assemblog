...
export default function Category() {
    ...
    useEffect(() => {
      if(refreshToken !== undefined) { // 리프레쉬 토큰이 있으면
    ...
    return (
        <>
          <CategoryHeader isLogin={isLogin} cookie={{email: email, refToken: refreshToken, accToken: accessToken}}/>
          <CategoryContents isLogin={isLogin} cookie={{email: email, refToken: refreshToken, accToken: accessToken}}/>
        </>
    )
}